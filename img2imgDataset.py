...
class ImageAndRefDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i, minimize=False, blur=0, s_size=128):
...
class Image2ImageDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, paths, root1='./input', root2='./target', dtype=np.float32, leak=(0, 0), root_ref = None, train=False):
...
class Image2ImageDatasetX2(Image2ImageDataset):
    ...
    def __getitem__(self, i, minimize=False, log=False, bin_r=0):
        path1 = os.path.join(self._root1, self._paths[i])
        path2 = os.path.join(self._root2, self._paths[i])
        #image1 = ImageDataset._read_image_as_array(path1, self._dtype)
        image1 = cv2.imread(path1, cv2.IMREAD_GRAYSCALE)
        ...
        _image1_2 = np.empty((_image1.shape[0], _image1.shape[1], _image1.shape[2] + 3), self._dtype)
        _image1_2[:, :, 0] = _image1[:, :, 0]
        _image1_2[:, :, 1:] = cv2.resize(
                image2, (_image1.shape[0], _image1.shape[1]), interpolation=cv2.INTER_CUBIC)

        return image1.transpose(2, 0, 1), _image1_2.transpose(2, 0, 1), _image2.transpose(2, 0, 1)
